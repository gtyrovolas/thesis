...
For every smart ballot $\vec{B}$ with delegating functions from $\LIQUID$ we can construct a directed graph that encodes this information $\varphi(\vec{B})$. In $\varphi(\vec{B})$ every arboresence is a valid certificate of the election induced by $\vec{B}$. This construction is essentially in \citet{grandi} and we formalise it below.

An arboresence $A$ in a directed graph $G = (V, E)$ rooted in $r$ is a subgraph for which for each $u \in V$  there exists a unique path from $r$ to $u$.
...
\begin{proposition}
    For every smart profile $\vec{B}$ there is a natural bijection between the consistent certificates of $\vec{B}$ and the arboresences of $\varphi(\vec{B})$ rooted in $r$. This is given by having voter $v$ delegate to their parent in the arboresence.
\end{proposition}

\begin{proof}
    Let $A$ an arboresence of $\varphi(\vec{B})$ rooted in $r$.
    Let voter $v$ delegate to their parent $\mathrm{par}(v)$.
    Then every agent is delegating to a unique agent by the properties of the arboresence.
    ...
    Then, $(d(u), u)$ forms are arboresence.
...
\begin{proof}
    ...
    Then it suffices to check if there exists an arboresence of $\varphi(\vec{B}_m)$.
    To do so, we can run Depth First Search from $r$ in $\mathcal{O}(nm)$ steps. As $m \leq l$, it is $\mathcal{O}(nl)$.
    If there is an arboresence it will be returned as the DFS tree of the traversal. If not, some node is unreachable from $r$ and thus $\vec{B}_m$ has no consistent certificate.
...
The equivalent graph theoretic question is, for graph $\varphi(\vec{B})$ is there a minimum weight arboresence with the size of subtree of $1$ greater than $k$? We leave whether this is polynomial time computable or NP-hard as an open question.
...
Now, let for an arboresence $T$, $T_0, T_1, T_*$ denote the subtrees of $0, 1, *$.

\begin{lemma}
    For a smart ballot $\vec{B}$ let $ G = \varphi(\vec{B})$. Suppose we construct $G'$ by adding to $\varphi(\vec{B})$ an edge $(d, v)$ with cost 1 for some $d \in D$ and $v \in N$. Then for each arboresence $A$ of $\varphi(\vec{B})$ there exists an arboresence $A'$ of $T'$ with $A_d \subseteq A'_d$ and for the other two symbols $x$ and $y$, $A'_x \subseteq A_x$ and $A'_y \subseteq A_y$.
\end{lemma}

\begin{proof}
    Let $A$ a minimum cost arboresence of $G$.
    We build $A'$ by removing edge $(\mathrm{par}(v), v)$ and adding edge $(d, v)$. 
    As $1$ is the minimum cost of any edge this weakly decreases the cost of $A$.
    The subtree $A \setminus A_v$ is a minimum cost arboresence of $G \setminus A_v$. 
    If that were not the case let $T$ an arboresence with cost strictly less than $A \setminus A_v$. 
    Then, $A_v \cup T \cup (\mathrm{par}(v), v)$ would be a strictly better arboresence than $A$ contradicting minimality of $A$ in $G$.
    If $v \notin A_d$ then $A'$ is a minimum cost arboresence.
...
\begin{proof}
    By the above lemma voting directly for a preferred alternative $d$ increases the votes for $d$ while decreases the votes for the other outcomes.
    As $r$ is monotone this implies that the outcomes in favour of $d$ cannot decrease.
