...
Firstly, every voter is allowed to vote directly on an issue. Additionally, delegations are \emph{transitive}. So that a scenario where Alice delegates to Bob and Bob delegates to Charlie is allowed. In this case Charlie votes with the combined power of all three.
...
\begin{proposition}
    Suppose that $r$ is monotone and $D = \{0, 1\}$. 
    Then for any outcome $d \in D$ there is an algorithm that determines if there is a \textsc{MinMax} certificate such that the election resolves in favour of $d$.
\end{proposition}

\begin{proof}
    By the results in \citet{grandi} there is an algorithm that returns a certificate that minimises the maximum of preferences used that runs in $\mathcal{O}(\abs{N}^2)$.
    Suppose the maximum preference level used in this certificate be $m$.
    Now, consider a directed graph $G = (V, E)$ with $V = N \cup \{0, 1\}$.
    For each $u \in N$, let voter $u$'s smart ballot be $B_u = (a_1 > \ldots > a_k)$.
    Then, we construct an edge $(a_i, u)$ for each $i \leq m$.

    We claim that every voter in the connected component of $d$ can vote at the same time for $d$.
    We run Breadth First Search starting from $d$.
    In the resulting BFS tree, $u$ votes for $v$ if there is an edge $(v, u)$ in the tree.
    Any agent $u$ with an edge $(d, u)$ can vote for $d$ directly by our construction. Then, every agent voting for $u$ would vote for $d$ indirectly and so on.
    This is a valid delegation as every agent has only one incoming edge in the BFS tree, this is equivalent to only voting for one alternative.
    Now, a voter $v$ not reachable from $0$ must be reachable from $1$.
    Otherwise $v$ cannot vote for any of the two options, which contradicts the fact that there is a \textsc{MinMax} certificate using only the first $m$ preferences.

    The above process returns a vector $\vec{x}$ maximal with respect to $d$.
    As $r$ is monotone the answer is equal to whether $r(\vec{x}) = d$.
